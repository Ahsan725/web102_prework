...
const fundingStatusStatement = `
  ${numberOfUnfundedGames > 0 ?
    `So far, ${numberOfUnfundedGames} game${numberOfUnfundedGames !== 1 ? 's' : ''} remain unfunded.` :
    'All games are fully funded!'
  }
  In total, $${GAMES_JSON.reduce((total, game) => total + game.pledged, 0).toLocaleString()} has been raised for ${GAMES_JSON.length} game${GAMES_JSON.length !== 1 ? 's' : ''}.
`;
// create a new DOM element containing the template string and append it to the description container
const paragraphElement = document.createElement('p');
paragraphElement.innerHTML = fundingStatusStatement;
...
topFundedGameElement.innerHTML = `
  <p>Name: ${topFundedGame.name}</p>
`;
// do the same for the runner up item
const secondFundedGameElement = document.createElement("div");
secondFundedGameElement.innerHTML = `
  <p>Name: ${secondFundedGame.name}</p>
`;
firstGameContainer.appendChild(topFundedGameElement);
secondGameContainer.appendChild(secondFundedGameElement);


//I am adding additional functionality of scrolling to the section when the user clicks on the link.
document.querySelector('a[href="#our-games-section"]').addEventListener('click', function(e) {
    e.preventDefault(); 
   
    document.querySelector('#our-games-section').scrollIntoView({ behavior: 'smooth' });
  });
